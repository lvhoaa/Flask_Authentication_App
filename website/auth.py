# this file also declares lots of routes 
...
@auth.route('/login',methods=['POST','GET']) # accept both post and get requests 
def login():
...
def sign_up():
    if request.method =='POST':
        ...
        # Message flashing: flash something on the screen
            ...
            # return redirect('/home'): but then if changed would need to adjust each link & redirect
            return redirect(url_for('auth.login')) # point the url to the function login in auth
            # must log in back after sign up

    
    # Notes: render_template at the end of function rather than the front
